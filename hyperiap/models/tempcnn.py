...
class TEMPCNN(torch.nn.Module):
    def __init__(
        ...
        self.conv_bn_relu1 = Conv1D_BatchNorm_Relu_Dropout(self.input_dim, self.hidden, kernel_size=self.kernel_size,
                                                           drop_probability=self.dropout)
        self.conv_bn_relu2 = Conv1D_BatchNorm_Relu_Dropout(self.hidden, self.hidden, kernel_size=self.kernel_size,
                                                           drop_probability=self.dropout)
        self.conv_bn_relu3 = Conv1D_BatchNorm_Relu_Dropout(self.hidden, self.hidden, kernel_size=self.kernel_size,
                                                           drop_probability=self.dropout)
        self.flatten = Flatten()
        self.dense = FC_BatchNorm_Relu_Dropout(self.hidden * self.seq_len, 4 * self.hidden, drop_probability=self.dropout)
...
class Conv1D_BatchNorm_Relu_Dropout(torch.nn.Module):
    def __init__(self, input_dim, hidden_dims, kernel_size=KERNEL_SIZE, drop_probability=DROPOUT):
